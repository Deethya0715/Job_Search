...
def _facts(profile: dict[str, Any], job: Any | None) -> str:
    ...
    return "\n".join(
        [
            ...
            f"Degree type: {profile.get('degree_type')}",
            f"Discipline: {profile.get('discipline') or 'Computer Science'}",
            ...
            f"Preferred work location: {profile.get('preferred_work_location')}",
            f"Has 3 months–2 years technical work (incl. internship) and can start full-time 2026: {profile.get('has_technical_work_3mo_to_2yr')}",
            f"US citizen: {auth.get('us_citizen')}",
            f"US person: {auth.get('us_person')}",
            ...
            f"Clearance level held: {(profile.get('security_clearance') or {}).get('level_held')}",
            f"Clearance eligible / U.S. person for export-controlled work: {(profile.get('security_clearance') or {}).get('us_person_and_clearance_eligible')}",
